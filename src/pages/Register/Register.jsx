...
const Register = () => {
  ...
  const { createUser, setUser, profile, googleLogin } = use(AuthContext);
  ...
  const handleRegister = (e) => {
    ...
    if (!passwordRegex.test(password)) {
      toast.error(
        "❌ Password must be At least one uppercase and one lowercase letter andMinimum 6 characters"
      );
      return;
    }
      ...
      <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-10 p-6 lg:p-10 text-white">
        ...
        <div className="w-full max-w-md backdrop-blur-lg bg-white/10 border border-white/20 shadow-2xl rounded-2xl p-8">
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            ...
            <button
              type="submit"
              className="btn text-white bg-gradient-to-r from-pink-500 via-red-400 to-yellow-400 w-full"
            >
              Register
            </button>
            {/* Google Signin */}
            <button
              ...
              className="btn text-white flex items-center justify-center gap-2 bg-gradient-to-r from-pink-500 via-red-400 to-yellow-400 px-5 py-2 rounded-lg w-full font-semibold  cursor-pointer"
            >
              <FcGoogle size={24} />
              Login with Google
            </button>
